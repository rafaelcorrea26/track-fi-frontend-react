...
import { Pencil, RefreshCw, Trash2, PlusCircle } from 'lucide-react'
...
type Props = {
  ...
  year: number
  onClose: () => void
  defaultEditId?: number
  defaultExtendId?: number
}
...
function toEditForm(r: RecurringTransaction): EditForm {
  return {
    ...
    months: r.months ? String(r.months) : '24',
  }
}

function durationLabel(r: RecurringTransaction) {
  if (!r.months) return 'Todo mês'
  return `${r.months} ${r.months === 1 ? 'mês' : 'meses'}`
}

function getLastMonth(r: RecurringTransaction): { month: number; year: number } | null {
...
function isExpiringAt(r: RecurringTransaction, m: number, y: number): boolean {
  const last = getLastMonth(r)
  return last !== null && last.month === m && last.year === y
}

export default function RecurringList({ month, year, onClose, defaultEditId, defaultExtendId }: Props) {
  const queryClient = useQueryClient()
  const [editing, setEditing] = useState<RecurringTransaction | null>(null)
  const [form, setForm] = useState<EditForm | null>(null)
  const [formError, setFormError] = useState('')
  const [extending, setExtending] = useState<RecurringTransaction | null>(null)
  const [extendMonths, setExtendMonths] = useState('24')
  ...
  useEffect(() => {
    if (!items.length || autoStarted.current) return
    if (defaultEditId) {
      const target = items.find(r => r.id === defaultEditId)
      if (target) { autoStarted.current = true; startEdit(target) }
    } else if (defaultExtendId) {
      const target = items.find(r => r.id === defaultExtendId)
      if (target) { autoStarted.current = true; setExtending(target) }
    }
  }, [defaultEditId, defaultExtendId, items])
  ...
  const extendMutation = useMutation({
    mutationFn: ({ id, months }: { id: number; months: number }) =>
      api(`/recurring-transactions/${id}/extend`, { method: 'POST', body: { months } }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['recurring-transactions'] })
      queryClient.invalidateQueries({ queryKey: ['transactions'], refetchType: 'all' })
      setExtending(null)
    },
    onError: (err) => setFormError(err instanceof ApiError ? err.message : 'Erro ao estender'),
  })
  ...
  function handleExtend(e: React.FormEvent) {
    ...
    if (isNaN(months) || months <= 0) { setFormError('Informe quantos meses'); return }
    extendMutation.mutate({ id: extending.id, months })
  }
    ...
    <div className="flex flex-col gap-4">
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-2">
          {items.map(r => {
            const expiring = r.active && isExpiringAt(r, month, year)
            const last = getLastMonth(r)

            return (
              <div key={r.id}>
                <div className={`flex items-center gap-3 p-3 rounded-lg border transition-opacity ${
                  r.active
                    ? expiring
                      ? 'bg-[hsl(38,92%,50%)]/5 border-[hsl(38,92%,50%)]/40'
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[hsl(215,20%,55%)] text-xs mt-0.5">
                      ...
                      {last && (
                        <span className={expiring ? ' · último mês' : ` · até ${MONTHS[last.month - 1]}/${last.year}`}>
                          {expiring ? ' · ' : ''}
                          {expiring && <span className="text-[hsl(38,92%,50%)] font-medium">último mês ⚠</span>}
                          {!expiring && ` · até ${MONTHS[last.month - 1]}/${last.year}`}
                        </span>
                      )}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {expiring && (
                      <button
                        onClick={() => { setExtending(r); setEditing(null); setForm(null); setFormError('') }}
                        title="Estender"
                        className="p-1.5 rounded-lg text-[hsl(38,92%,50%)] hover:bg-[hsl(38,92%,50%)]/10 transition-colors"
                      >
                        <PlusCircle size={14} />
                      </button>
                    )}
                ...
                {/* form de extensão inline */}
                {extending?.id === r.id && (
                  <form
                    onSubmit={handleExtend}
                    className="mt-1 p-4 rounded-lg border border-[hsl(38,92%,50%)]/30 bg-[hsl(222,20%,9%)] flex flex-col gap-3"
                  >
                    <p className="text-[hsl(38,92%,50%)] text-xs font-medium">
                      Estender <strong>{r.description}</strong> — adicionar meses a partir de{' '}
                      {last ? `${MONTHS[last.month - 1]}/${last.year}` : 'agora'}
                    </p>

                    {formError && <p className="text-[hsl(0,84%,60%)] text-xs">{formError}</p>}

                    <div className="flex items-center gap-3">
                      <div className="flex flex-col gap-1">
                        <label className="text-[hsl(215,20%,55%)] text-xs">Adicionar meses (máx 120)</label>
                        <div className="flex items-center gap-2">
                          <input
                            type="number" min="1" max="120"
                            value={extendMonths}
                            onChange={e => setExtendMonths(e.target.value)}
                            className="bg-[hsl(217,20%,14%)] border border-[hsl(217,20%,18%)] rounded-lg px-3 py-2 text-[hsl(210,40%,96%)] text-sm outline-none focus:border-[hsl(38,92%,50%)] w-24 transition-colors"
                          />
                          <span className="text-[hsl(215,20%,45%)] text-xs">meses</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2">
                      <button
                        type="button"
                        onClick={() => { setExtending(null); setFormError('') }}
                      ...
                      <button
                        ...
                        className="flex-1 py-2 rounded-lg bg-[hsl(38,92%,50%)] text-[hsl(30,100%,6%)] font-semibold text-sm hover:brightness-110 transition-all disabled:opacity-50"
                      >
                        {extendMutation.isPending ? 'Estendendo...' : 'Estender'}
                      </button>
                    </div>
                  </form>
                )}
